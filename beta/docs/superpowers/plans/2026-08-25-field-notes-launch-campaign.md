...
>
> **Correction (attribution/voice):** the series was originally drafted in bumbleflies "we" voice, describing the system as built "for a legal-tech client." bumbleflies has never worked for JUNE as a client — Chris built and runs this system in his own role at JUNE, separate from his work at bumbleflies. All 12 posts, both blog listing hero blurbs, and the JSON-LD `author` field were rewritten to first-person ("I built this at JUNE...") with a one-time, non-promotional disclosure in post 1 clarifying the dual affiliation. Byline is `Chris 🦋 | Founder at bumbleflies / Senior Product Manager at JUNE`; JSON-LD `author` is `Christian Dähn` (Person), `publisher` stays `bumbleflies` (Organization) since bumbleflies is the site's legitimate publisher, just not the system's builder-for-hire.
>
> **Correction (terminology & voice), 2026-09-02:** external review ([GitHub #176](https://github.com/bumbleflies/web/issues/176)) on post 1 found series-wide inconsistencies, not just a Teil-01 wording slip. Fixed across all 12 files: (1) **"Ebene"/"Schicht"** (2 foundation "levels" vs. 4 agent "layers" — near-synonyms in German) renamed to **"Fundament"/"Säule"** (2 Fundamente, 4 Säulen — "Foundation"/"Pillar" in English), following the numbered-label pattern the article already used ("Fundament 1, der Zustand." / "Säule 1, das Nervensystem."). (2) **"Bot" dropped entirely**, unified on **"Agent"/"Agenten"** everywhere (Teil 05 hit hardest — it's literally about bots — including its title, now "Agenten, die arbeiten, während du schläfst" / "Agents That Work While You Sleep"). (3) Stray formal **"Sie"/"Ihr" address** fixed to "du" in the 3 DE posts that had it (Teil 01, 03, 04) — the series otherwise never addresses the reader directly, so this was a targeted fix of existing direct-address sentences, not a full voice rewrite. (4) AI-buzzword **"fundamental"** and jargon **"dedupliziert"/"entdupliziert"** replaced with plain language in Teil 01 (also avoids colliding with the new "Fundament" term). (5) Added a hand-authored inline architecture diagram (dark/light SVG, same brand honey-amber palette as the cover art) to Teil 01 illustrating "zwei Fundamente, vier Säulen," embedded via raw HTML + a new `.a-arch-diagram` CSS class in `design-system.css` (same theme-swap technique as `BlogCover.astro`, but global since it's not scoped to a component). **For future posts in this series:** use "Agent" (never "Bot"), "Fundament"/"Säule" for the 2-foundation/4-pillar architecture, and keep any direct reader-address in "du," not "Sie."

## Context

The "Field Notes" series — 6 posts × DE/EN (12 files in `beta/src/content/blog/`) — is fully written: a real personal account of building a production AI-agent system at JUNE, a German legal-tech company where the author (Chris) works alongside his role at bumbleflies — not a bumbleflies client engagement (agent OS → state/interaction architecture → n8n event automation → skill marketplace → autonomous Claude Code daemon → a personal "cockpit" orchestrating ten agents). It has strong narrative arc and credibility (it's a built system, not a pitch deck), but right now it has **zero reach**: all 12 posts are `published: false`, there's no way to share a post that looks good on LinkedIn (no per-post social image), no RSS feed for anyone to subscribe to, and no share affordances on the post pages themselves. The goal is to fix both halves of that problem — give the series a real distribution push, and make the site capable of supporting that push — timed to a launch starting **2026-09-01**.
