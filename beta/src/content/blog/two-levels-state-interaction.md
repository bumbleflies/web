---
title: "The Two Foundations Everything Runs On: State and Interaction"
...
- **The comment command bus.** Comments whose first word is a fixed control word become commands. A human can type them, an agent can post them, and both are forever logged in the ticket. The entire release pipeline is controlled through this one, auditable channel, no separate dashboard, no hidden API.
...
- It's the **sole trigger** for autonomous agents. No webhook, just a simple 30-second poll looking for a trigger word. That sounds primitive, but it's robust: no webhook registration that can break, no externally exposed interface.
